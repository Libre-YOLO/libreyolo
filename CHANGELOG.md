...
- Native fp8 execution tier: finalized fp8 QuantLinear runs on the fp8 tensor cores via torch._scaled_mm (new `fp8_gemm` registry kernel, Ada/Hopper/Blackwell), finalized fp8 QuantConv2d convolves in fp16 on cached dequantized weights, and fp16-remainder finalized checkpoints get the cast recipes' float32 I/O root hooks on load (previously they crashed on fp32 input); LibreFeyNobg fp8 drops 484 -> 217 ms eager, 128 ms graphed
