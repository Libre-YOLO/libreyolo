...
- `general_nightly`: one smallest native inference case for every public
  ...
  auto-download route (LibreYOLO HF, or Deci's CDN for YOLO-NAS), plus a
  task-appropriate DeepLabv3 semantic stability case, batched/sequential
  detector parity, and selected open-vocabulary smoke cases; currently 37
  tests.
...
- Detector families cover detection, while DeepLabv3 covers dense semantic
  masks. L2CS gaze is non-redistributable (no public download route), so it
  runs as a non-gated per-family suite
