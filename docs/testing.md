...
- `general_nightly`: one smallest native inference case for every public model
  ...
  inference-only museum families, that has a public auto-download route
  (LibreYOLO HF, or Deci's CDN for YOLO-NAS), plus batched/sequential parity
...
- Detector families cover detection and AlexNet covers classification. L2CS
  gaze is non-redistributable (no public download route), so it runs as a
