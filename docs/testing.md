...
- `general_nightly`: one smallest native inference case for every public model
  family, including inference-only museum and classification families, that has
  a public auto-download route (LibreYOLO HF, or Deci's CDN for YOLO-NAS), plus
  task-appropriate batched/sequential parity and selected open-vocabulary smoke
  cases; currently 38 tests.
