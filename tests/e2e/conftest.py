...
# Full legacy export test set. Faster R-CNN and CenterNet have narrow,
# family-specific export contracts and dedicated runtime parity gates, so
# blocked formats must not attempt them merely because their public weights
# are in MODEL_CATALOG.
FULL_TEST_MODELS = [
    (family, size)
    for family, size in NON_RFDETR_MODELS
    if family not in {"faster_rcnn", "centernet"}
]
