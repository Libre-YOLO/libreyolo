...
# Full legacy export test set. Faster R-CNN and Mask R-CNN are ONNX-only and
# have official-checkpoint runtime parity gates, so blocked formats must not
# attempt to export them merely because their public weights are in MODEL_CATALOG.
FULL_TEST_MODELS = [
    (family, size)
    for family, size in NON_RFDETR_MODELS
    if family not in {"faster_rcnn", "mask_rcnn"}
]
